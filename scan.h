...
class cScan:public cThread
{
  private:
    ...
    int ScanServices(bool noSDT = false);
    ...
    void ScanATSC(cTransponder * tp, cChannel * c);
    void ScanDVB_I(cTransponder * tp, cChannel * c);
    void ScanAnalog(cTransponder * tp, cChannel * c);

    void AddTransponders(int system);
    void ClearMap();
  ...
  public:
      cScan();
     ~cScan();
    int CardNR() const {return cardnr;}
    void ShutDown();
    ...
    const char *GetCurrentParameters() const
    {
        return parameters;
    }
